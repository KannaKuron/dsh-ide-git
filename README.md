...
| 区域 | 能力 |
|---|---|
| 工具条 | 刷新、新建分支、Fetch(默认 prune)、Pull(`--ff-only`)、Push(二次确认)、分支栏开合 |
| 状态行 | 当前分支、上游、`↑ahead ↓behind`、stash 数量、忙碌指示 |
| 分支树 | `HEAD / 本地 / 远程 / 标签` 分组(可折叠)、过滤框、星标当前分支、ahead/behind 角标、worktree 占用标记 |
| 分支右键 | 签出、将当前分支变基到此、合并到当前分支、与当前分支比较、从该分支新建分支、重命名、删除(当前分支禁用)、更新(Fetch)、推送 |
| 提交列表 | lane 拓扑图、refs 徽章(HEAD / 分支 / 标签)、相对时间、作者、主题、过滤、加载更多(每页 120) |
