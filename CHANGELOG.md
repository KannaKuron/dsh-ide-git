...
- **验证**:`npm test` 70 项(新增 9 项:分桶与 panes.v1 键名、`normalizePanes` 边界、clamp 上下限与"挤不成 0"、分隔条 `touch-action:none`/指针捕获/键盘/双击、浮层宽度夹取、浮层文案键逐语言齐、dock 动作可用性(服务缺失/半残/浮起时 split 关闭)、树开关两向可达、dock 只在原生座位接线);`scripts/ssr-check.mjs` 三态通过;`scripts/layout-probe.mjs` 退出码 0 且 11 档 chrome 与改动前**逐档一致**(不变量 7 零漂移);新增 `scripts/panes-probe.mjs`(34 项)与 `scripts/dock-probe.mjs`(原生门 23 项 / 底座降级门 17 项)真机断言全绿、零 pageerror——停靠侧含"浮动后浮层里就是本面板、按钮变『收回停靠』""收回后回到 pane""分栏 panes=2/dividers=1""全屏模式切换""紧凑头部树按钮收起后仍在且能再展开"——含"拖拽确实改变尺寸 325→396px""reload 后 396px""新会话后 396px""宽扁桶(columns:wide)与窄高桶(stack:tall)互不影响""199px 面板里菜单 191px / 对话框 183px 都在 `.dig-root` 内"。
