...
- **删除可撤回(真删 + 浮窗撤回)**:删除分支 / 删除贮藏后,面板右下角浮出提示条,点「撤回」就能恢复。删除本身仍然是**真删除**(`git branch -d` / `git stash drop`),宿主在动手前记下对象 id,撤回时用 `git branch <name> <sha>` / `git stash store` 依据该 id 重建。撤回是**一次性**的:用过后句柄立即失效(重放返回 `undo-gone`),分支名被重新占用时明确返回 `undo-conflict`,句柄 30 分钟后过期,每个仓库最多保留 20 条。删除响应里直接带 `undo: { id, kind, label }`,客户端不需要再查一次接口。
